...
local CreateInventorySlot = function(dmhud, options)
		...
		canDragOnto = function(element, target)
			...
			elseif target.data.equipmentSlot ~= nil then
				...
				if slotInfo.type == 'armor' and item.type == 'Armor' then
					return true
				elseif item.type == 'Weapon' and slotInfo.loadout ~= nil then
					return true
				elseif item.type == 'Shield' and slotInfo.loadout ~= nil and slotInfo.main ~= true then
					return true
				elseif item:try_get('requiresAttunement') and slotInfo.attune then
					return true
				elseif item:try_get('magicalItem') and (not item:try_get('requiresAttunement')) and slotInfo.accessory and
				       item.type ~= "Armor" and item.type ~= "Weapon" and item.type ~= "Shield" then
					return true
				elseif slotInfo.loadout ~= nil and item:CanWield() then
					return true
				elseif slotInfo.accessory and item:try_get('equipOnBelt', false) then
					return true
				end
			else
				return target.data.inventoryDragTarget
	...
	slotPanel = gui.Panel{
		...
		events = {
			...
			click = function(element)
				element.popup = nil
				dmhub.Debug(string.format("OPEN:: CLICK: %s %s", dmhub.ToJson(accessParty), dmhub.ToJson(token ~= nil)))
				if adding then
					...
					element.popup = gui.ContextMenu {
						entries = {
							...
							},
							{
								...
								click = function()
									element.popup = nil
									dmhud.createItemDialog.data.show(dmhud.basicInventoryDialog, item)
								end,
							},
							{
								text = 'Duplicate Item',
						...
						contextMenuItems[#contextMenuItems+1] =
							{
								...
								click = function()
									...
									gamehud.basicInventoryDialog:FireEventTreeVisible("newItem", itemid)
								end,
							}
					end
		...
		data = {
			...
			SetSlot = function(info, tok, dialog)
				...
				if info == nil then
					...
					slotPanel.bgimage = 'panels/InventorySlot_Background.png'
					item = nil
					SetRarity(nil)
				else
...
function GameHud.CreateInventoryDialog(self, options)
	...
	local categories = {"All", "Weapon", "Armor", "Gear", "Consumable" }
	local categoriesPlural = {"All", "Weapons", "Armor", "Gear", "Consumables" }
	local catIndex = 1
	...
	local slotsContainer = gui.Panel{
		...
		events = {
			...
			refreshInventory = function(element)
				...
				if categories[catIndex] == "Consumable" then
					for k,entry in pairs(unarrangedInventory) do
						if gearTable[k] ~= nil and gearTable[k]:has_key("consumable") then
					...
					local catMatch = categoryMatches[categories[catIndex]]
					for k,entry in pairs(unarrangedInventory) do
						if gearTable[k] ~= nil and catMatch[gearTable[k].type] then
							sortedInventory[#sortedInventory+1] = {
								id = k,
								entry = entry,
								item = gearTable[k],
	...
	local magicItemsCheck = nil
	local treasureCheck = nil
	...
	if basicInventory then
		...
		magicItemsCheck = gui.Check{
			text = "Show Magic Items",
			style = {
				height = 14,
				fontSize = 10,
				width = "auto",
			...
			value = dmhub.GetSettingValue("inventory:magicalitems"),
			change = function(element)
				dmhub.SetSettingValue("inventory:magicalitems", element.value)
				resultPanel:FireEventTree('refreshInventory')
			end,
		}

		treasureCheck = gui.Check{
			text = "Show Treasure",
			style = {
				height = 14,
				fontSize = 10,
			...
			value = dmhub.GetSettingValue("inventory:treasure"),
			change = function(element)
				dmhub.SetSettingValue("inventory:treasure", element.value)
				resultPanel:FireEventTree('refreshInventory')
			end,
		}
				...
				gui.Label{
					...
					events = {
						refreshInventory = function(element)
							...
							element.text = categoriesPlural[catIndex]
						end,
					}
				},
	...
	if tradeInventory then
		takeAllButton = gui.IconButton{
			icon = 'ui-icons/Back.png',
			style = {
				bgcolor = 'white',
				color = 'white',
				height = 32,
		...
		events = {
			...
			refreshInventory = function(element)
				...
				if basicInventory then
					...
					local showMagicalItems = dmhub.GetSettingValue("inventory:magicalitems")
					local showTreasure = dmhub.GetSettingValue("inventory:treasure")
					inventory = {}
					for k,item in pairs(gearTable) do
						if (not item:has_key('hidden')) and (not item:has_key('uniqueItem')) and (dmhub.isDM or not item:try_get('hiddenFromPlayers', itemsHiddenByDefault)) and (showMagicalItems or (not item:has_key("magicalItem"))) and (showTreasure or (not EquipmentCategory.IsTreasure(item))) then
							inventory[k] = {
								quantity = 1,
							}
						end
		...
		data = {
			...
			open = function(tok, options)
				...
				if options.showBasic then
					self.tradeInventoryDialog.data.close()
					self.basicInventoryDialog.data.open()
				end
			...
			close = function()
				...
				if playerInventory then
					...
					self.basicInventoryDialog.data.close()
					self.tradeInventoryDialog.data.close()
				end
			end,
		},
...
local CreateEquipmentSlot = function(dmhud, options)
	...
	local iconPanel = gui.Panel{
		id = 'inventory-slot-icon',
		bgimage = 'ui-icons/slot-' .. creature.EquipmentSlots[options.slot].icon .. '.png',
		draggable = false,
		...
		data = {
			...
			SetItem = function(newItem, shadow)
				...
				if item then
					...
					iconPanel.bgimage = 'ui-icons/slot-' .. creature.EquipmentSlots[options.slot].icon .. '.png'
...
function GameHud.CreateEquipmentDialog(self, options)
	...
	local dialogHeight = cond(options.charsheet, 400, 680)
	...
	local mainhandSlots = {
		CreateEquipmentSlot(self, { slot = 'mainhand1' }),
		CreateEquipmentSlot(self, { slot = 'mainhand2' }),
		CreateEquipmentSlot(self, { slot = 'mainhand3' }),
	}

	local offhandSlots = {
		CreateEquipmentSlot(self, { slot = 'offhand1' }),
		CreateEquipmentSlot(self, { slot = 'offhand2' }),
		CreateEquipmentSlot(self, { slot = 'offhand3' }),
	}


	local armorSlot = CreateEquipmentSlot(self, { slot = 'armor' })


	local beltSlots = {
		CreateEquipmentSlot(self, { slot = 'belt1' }),
		CreateEquipmentSlot(self, { slot = 'belt2' }),
		CreateEquipmentSlot(self, { slot = 'belt3' }),
		CreateEquipmentSlot(self, { slot = 'belt4' }),
	}


	local attuneSlots = {
		CreateEquipmentSlot(self, { slot = 'attune1' }),
		CreateEquipmentSlot(self, { slot = 'attune2' }),
		CreateEquipmentSlot(self, { slot = 'attune3' }),
		CreateEquipmentSlot(self, { slot = 'attune4' }),
	}

	allSlots[#allSlots+1] = armorSlot
	for i,slot in ipairs(mainhandSlots) do
		allSlots[#allSlots+1] = slot
	end
	for i,slot in ipairs(offhandSlots) do
		allSlots[#allSlots+1] = slot
	end
	for i,slot in ipairs(beltSlots) do
		allSlots[#allSlots+1] = slot
	end
	for i,slot in ipairs(attuneSlots) do
		allSlots[#allSlots+1] = slot
	end

	local leftEquipmentSlotsPanel = gui.Panel{
		style = {
			height = 'auto',
			width = 'auto',
			flow = 'vertical',
	...
	local armorSlotPanel = gui.Panel{
		style = {
			height = 'auto',
			width = 'auto',
			flow = 'vertical',
	...
	local rightEquipmentSlotsPanel = gui.Panel{
		style = {
			height = 'auto',
			width = 'auto',
			flow = 'vertical',
		},
		children = offhandSlots,
	}

	local slotsArea = gui.Panel{
		style = {
			width = '100%',
			height = 'auto',
			flow = 'horizontal',
		...
		children = {
			leftEquipmentSlotsPanel,
			armorSlotPanel,
			rightEquipmentSlotsPanel,
		}
	}


	local beltSlotsArea = gui.Panel{
		style = {
			width = '90%',
			height = 'auto',
			flow = 'horizontal',
	...
	local attunementSlotsArea = gui.Panel{
		style = {
			width = '90%',
			height = 'auto',
			flow = 'horizontal',
		},
		children = attuneSlots,
	}
