...
function creature.TakeDamage(self, amount, note, info)
	...
	local eventArg = shallow_copy_table(info or {})
	...
	if (not isDyingAtStart) and self:IsDeadOrDying() then
		...
		if self:IsDeadOrDying() then
			...
			if eventArg.attacker ~= nil then
				eventArg.attacker:DispatchEvent("kill", eventArg)
			end
	...
	if info ~= nil and info.attacker ~= nil then
	...
	local append = statHistory.Append
	self:GetStatHistory("hitpoints"):Append{
...
function creature:IsDeadOrDying()
	return self:IsDown() and self:GetNumDeathSavingThrowSuccesses() < 3
end

function creature:IsDying()
	return self:IsDeadOrDying() and (not self:IsDead())
end

function creature:IsDead()
	return self:IsDeadOrDying() and self:GetNumDeathSavingThrowFailures() >= 3
end
...
--any kind of no hitpoints. Dead/dying/stable.
...
creature.helpSymbols = {
	...
	type = {
		name = "Type",
		type = "text",
		desc = "The type of the creature. For instance, humanoid or undead for monsters, or Elf or Human for characters.",
		seealso = {"Subtype"},
	...
	chargedistance = {
		...
		type = "number",
		desc = "The distance this creature has moved in a straight line to get to its current position, in feet. Only counts movement made on the current turn, and will always be 0 when not in combat. A mix of diagonal and straight-line movement will be counted as part of the charge as long as every step takes the creature closer to the target.",
		seealso = {},
		examples = {"Charge Distance >= 20"},
	},
	...
	distance = {
		name = "Distance",
		type = "function",
		desc = "A function which is shown another creature and tells us the distance between them in feet.",
	...
	countnearbyenemies = {
		...
		type = "function",
		desc = "A function which is shown a distance in feet and tells us the number of enemy creatures within that distance of this creature.",
		examples = {"OBJ.Count Nearby Enemies(5)"},
	},

	countnearbyfriends = {
		name = "Count Nearby Friends",
		type = "function",
		desc = "A function which is shown a distance in feet and tells us the number of friendly creatures within that distance of this creature.",
	...
	countnearbycreatures = {
		...
		type = "function",
		desc = "A function which is shown a distance in feet and tells us the number of creatures within that distance of this creature.",
		examples = {"OBJ.Count Nearby Creatures(5)"},
	},
	...
	movementspeed = {
		name = "Movement Speed",
		type = "number",
		desc = "The distance this creature can move in a round, in feet.",
	},
...
creature.lookupSymbols = {
	...
	type = function(c)
		return c:try_get("monster_category", "monster")
	end,
	...
	countnearbyenemies = function(c)
		return function(distance)
			...
			for i,nearby in ipairs(nearbyTokens) do
				if not nearby:IsFriend(token) then
					count = count+1
				end
			end
	...
	countnearbycreatures = function(c)
		return function(distance)
			local token = dmhub.LookupToken(c)
			if token == nil then
				return 0
...
for _,movementType in ipairs(creature.movementTypeInfo) do
	...
	creature.helpSymbols[key] = {
		name = name,
		type = "number",
		desc = string.format("The %s of the creature, in feet per round. 0 if the creature does not have a %s.", name, name),
		seealso = seealso,
	}
end
