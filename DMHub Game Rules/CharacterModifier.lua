...
CharacterModifier.TypeInfo.attribute = {
	...
	createEditor = function(modifier, element)
		...
		Refresh = function()
			...
			for _,option in ipairs(m_options) do
				if (catChosen == nil or catChosen == option.category) and (not option.hidden) then
...
CharacterModifier.RegisterType('resistance', "Resistance")
...
CharacterModifier.TypeInfo.resistance = {
	...
	createEditor = function(modifier, element)
		...
		Refresh = function()
			...
			local optionsTable = {}
			for i,damageType in ipairs(rules.damageTypes) do
				optionsTable[damageType] = true
			end

			for i,r in ipairs(modifier.resistances) do
				optionsTable[r.damageType] = nil
			end
...
CharacterModifier.TypeInfo.transform = {
	...
	getNamedResources = function(modifier, creature, resourceTable)
		...
		if creature:has_key("transformInfo") then
			local monsterInfo = assets.monsters[creature.transformInfo.transformid]
			local beast = monsterInfo.properties
			local beastDice = beast:GetHitDiceMaximums()
			for k,quantity in pairs(beastDice) do
				resourceTable[k] = quantity
			end
		end
...
function CharacterModifier:ConsumeResourceInternal(creature, modContext)
	...
	creature:ConsumeResource(self.guid, refreshType)
	return true
end
...
function CharacterModifier:HasResourcesAvailable(creature)
	...
	return creature:GetResourceUsage(self.guid, refreshType) < self:GetNumberOfCharges(creature)
end

function CharacterModifier:DescribeResourceAvailability(creature)
	local refreshType = self:GetResourceRefreshType()
	if refreshType == 'none' or refreshType == 'perspell' then
		local resourceCost = creature:ResourceToConsume(self.resourceCost)
		if resourceCost ~= nil then
	...
	local used = creature:GetResourceUsage(self.guid, refreshType)
...
function CharacterModifier:HasTriggeredEvent(creature, eventName, targetsOther)
	if self:has_key('triggeredAbility') and self.triggeredAbility.trigger == eventName then
        ...
        if subject ~= "self" and subject ~= "any" and subject ~= "selfandallies" and not targetsOther then
            return false
        end

        if not self.triggeredAbility:subjectHasRequiredCondition(creature, creature) then
            return false
        end
        
		if self:HasResourcesAvailable(creature) == false or (not self.triggeredAbility:CanAfford(creature)) or ((not self.triggeredAbility.mandatory) and (not creature:TriggeredAbilityEnabled(self.triggeredAbility))) then
			return false
		end
		return true
	else
...
function CharacterModifier:TriggerEvent(creature, eventName, info, modContext)
	if self:has_key('triggeredAbility') and self.triggeredAbility.trigger == eventName then
        if self.triggeredAbility:try_get("whenActive", "always") == "combat" and (dmhub.initiativeQueue == nil or dmhub.initiativeQueue.hidden) then
            return
        end
		...
		self.triggeredAbility:Trigger(self, creature, info, nil, modContext)
		return true
	end
...
--this accumulates duplicate proficiencies in skills and tools tables, which are maps of string -> count.
--this is used to allow the player to choose different skills/tools instead.
function CharacterModifier:AccumulateDuplicateProficiencies(skills, tools)
	local typeInfo = CharacterModifier.TypeInfo[self.behavior] or {}
	local accumulateDuplicateProficiencies = typeInfo.accumulateDuplicateProficiencies
	if accumulateDuplicateProficiencies ~= nil then
		accumulateDuplicateProficiencies(self, skills, tools)
	end
end
...
function CharacterModifier:PassesFilter(creature)
	if not self:has_key("filterCondition") or self.filterCondition == "" then
		return true
	end
...
function CharacterModifier:ModifyRollProperties(context, creature, rollProperties)
	local typeInfo = CharacterModifier.TypeInfo[self.behavior] or {}
	if typeInfo.modifyRollProperties ~= nil then
		self:InstallSymbolsFromContext(context)
		typeInfo.modifyRollProperties(self, creature, rollProperties)
	end
end