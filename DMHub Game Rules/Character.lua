...
function character:GetDarkvision()
	...
	local race = self:Race()
	if race ~= nil then
		local info = race:try_get('darkvision')
		if info ~= nil then
			darkvision = info.range or 0
		end
	end
...
function character:GetClassesAndSubClasses()
	...
	for i,entry in ipairs(classes) do
		...
		if classInfo ~= nil then
			...
			local subclasses = classInfo:GetSubclasses(self:GetLevelChoices(), entry.level)
...
function character:GetClassLevels()
	...
	for i,entry in ipairs(classes) do
		entry.class:FillLevelsUpTo(entry.level, i ~= 1, result)
	end
...
function character:IsDying()
	return self:CurrentHitpoints() <= 0 and self:GetNumDeathSavingThrowSuccesses() < 3 and self:GetNumDeathSavingThrowFailures() < 3
end

function character:IsDead()
	return self:CurrentHitpoints() <= 0 and self:GetNumDeathSavingThrowFailures() >= 3
end
