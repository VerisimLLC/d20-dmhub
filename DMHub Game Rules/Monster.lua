...
function monster.OnCreateFromBestiary(self, token)
	...
	if dmhub.GetSettingValue("randomizeMonsters") then
		self:RerollHitpoints()
	else
		self.max_hitpoints = math.floor(math.max(1, dmhub.RollExpectedValue(self.max_hitpoints_roll)))
	end
	...
	if settingAssignMonstersNames:Get() and self:has_key("monster_type") then
...
function monster.RerollHitpoints(self)
	self.max_hitpoints = dmhub.RollInstant(self.max_hitpoints_roll)
...
function monster:GetNameGeneratorTable()
	...
	if self:has_key("monster_type") then
		for k,v in pairs(nameDataTable) do
			if string.starts_with(string.lower(v.name), string.lower(self.monster_type)) then
				return v
			end
		end
	end
...
function monster:RaceOrMonsterType()
	return self:GetMonsterCategoryList()[1]
end
...
function monster:GetHitDiceMaximums()
	local result = {}

	if self:has_key("max_hitpoints_roll") then
		local info = dmhub.ParseRoll(self.max_hitpoints_roll)
		local categories = info.categories
		for _,cat in pairs(categories) do
			for _,group in ipairs(cat.groups) do
				local resourceid = string.format("hitDie%d", group.numFaces)
				result[resourceid] = (result[resourceid] or 0) + group.numDice
			end
		end
	end
