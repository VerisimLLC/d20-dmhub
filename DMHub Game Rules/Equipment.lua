...
function equipment:FillWornArmorModifiers(creature, result)
	if self:try_get("stealth") == "Disadvantage" then
		local cond = CharacterCondition.conditionsByName["Clunky Armor"]
		if cond ~= nil then
...
function equipment:Render(options, token)
	...
	if token ~= nil and token.properties ~= nil then
		...
		if cat ~= nil and (cat.allowProficiency or cat.allowIndividualProficiency) and token.type == "token" and token.properties.typeName ~= "monster" then
