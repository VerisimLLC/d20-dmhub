			...
			events = {
                ...
                searchCompendium = function(element, text)
                    ...
                    if text == "" then
                        ...
                        if options.contentType ~= nil then
                            element:SetClass("matchSearch", #SearchTableForText(dmhub.GetTable(options.contentType), text) > 0)
	                    elseif options.tableName ~= nil and options.key ~= nil then
		                    local table = dmhub.GetTable(options.tableName)
		                    local item = table[options.key]
...
local ShowOngoingEffectsPanel = function(parentPanel)

	local ongoingEffectPanel = CharacterOngoingEffect.CreateEditor()
	...
	itemsListPanel = gui.Panel{
		...
		refreshAssets = function(element)
			...
			for k,item in pairs(ongoingEffectTable) do
				newOngoingEffectItems[k] = ongoingEffectItems[k] or CreateListItem{
					select = element.aliveTime > 0.2,
					tableName = "characterOngoingEffects",
					key = k,
					click = function()
						SetOngoingEffect(k)
					end,
	...
	local leftPanel = gui.Panel{
		...
		AddButton{
			...
			click = function(element)
				dmhub.SetAndUploadTableItem("characterOngoingEffects", CharacterOngoingEffect.Create())
			end,
		}
	}
		...
		refreshCode = function(element)
			...
			for _,modid in ipairs(code.loadedMods) do
				...
				newDataItems[modid] = dataItems[modid] or CreateListItem{
					...
					search = function(element, str)
						if str == nil or str == "" then
							...
							for i,file in ipairs(mod.files) do
								if file:MatchesSearch(str, { ignorecase = ignorecase}) then
									match = true
									break
								end
							end
					...
					modified = function() return mod.checkedout end,
					click = function()
...
local LibraryPanel = function()
	...
	resultPanel = gui.Panel{
        ...
        gui.Panel{
            ...
            --- @param element Panel
            click = function(element)
                local dialog = element:FindParentWithClass("framedPanel")
                if dialog ~= nil then
                    element:SetClass("hidden", true)
...
dmhub.RegisterEventHandler("refreshTables", function(keys)
    ...
    Compendium.Register{
        ...
        click = function(contentPanel)
            ShowRacesPanel(contentPanel, "subraces")
        end,
    }
    ...
    Compendium.Register{
        ...
        contentType = "characterOngoingEffects",
        click = function(contentPanel)
            ShowOngoingEffectsPanel(contentPanel)
        end,
    }
