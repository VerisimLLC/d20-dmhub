...
mod.shared.InventoryCompendiumEditor = function()
    ...
    local showMagicItems = dmhub.GetSettingValue("compendium:showmagicitems")

    local magicItemsCheck = gui.Check{
        text = "Show Magical Items",
        value = showMagicItems,
        change = function(element)
            dmhub.SetSettingValue("compendium:showmagicitems", element.value)
            showMagicItems = element.value
            resultPanel:FireEventTree("refreshInventory")
        end,
    }

    local showTreasure = dmhub.GetSettingValue("compendium:showtreasure")

    local treasureCheck = gui.Check{
        text = "Show Treasure",
        value = showTreasure,
        change = function(element)
            dmhub.SetSettingValue("compendium:showtreasure", element.value)
            showTreasure = element.value
            resultPanel:FireEventTree("refreshInventory")
        end,
    }
		...
		refreshInventory = function(element)
			...
			for k,item in pairs(itemsTable) do
				...
				if (not item:try_get("hidden", false)) and
                   ...
                   (dmhub.isDM or (not item:try_get("hiddenFromPlayers", itemsHiddenByDefault))) and
                   (showMagicItems or (not item:has_key("magicalItem"))) and
                   (showTreasure or (not EquipmentCategory.IsTreasure(item))) then
