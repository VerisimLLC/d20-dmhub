					...
					gui.Label({
						...
						events = {
							change = function(element)
								...
								if creature.typeName == "monster" and element.text ~= "" then
							...
							refreshToken = function(element, info)
								local creature = info.token.properties
								element.editable = creature.typeName == "monster"
						...
						gui.Panel{
							...
							refreshToken = function(element, info)
								element:SetClass("hidden", info.token.properties.typeName ~= "monster")
							end,
					...
					local row = rowsCache[skillInfo.id] or gui.Panel{
						...
						gui.Panel{
							...
							gui.Panel{
								...
								click = function(element)
									...
									local isMonster = info.token.properties.typeName == "monster"
						...
						gui.Label{
							...
							refreshToken = function(element, info)
								...
								element.editableOnRightClick = (info.token.properties.typeName == "monster")
							end,
							press = function(element)
...
local EditResistanceEntry = function(creature, resistanceEntry, params)
	...
	local args = {
		...
		children = {
			...
			gui.Dropdown({
				options = rules.damageTypesAvailableWithAll,
...
function CharSheet.CharacterSheetEditPassiveSensesPopup(element, info)
	...
	--DARKVISION
	if creature.typeName == "monster" then
		children[#children+1] = gui.Panel{
			classes = {"statsRow", "passiveSenses"},
	...
	resultPanel = gui.Panel{
		...
		gui.Label{
			...
			refreshToken = function(element, info)
				...
				if info.token.properties.typeName == "monster" and element.text == "" then
		...
		gui.Label{
			...
			refreshToken = function(element, info)
				if info.token.properties.typeName == "monster" then
				...
				element:SetClass("editableLabel", info.token.properties.typeName == "monster")
			end,
		...
		gui.Label{
			...
			editable = true,
			refreshAppearance = function(element, info)
				element:SetClass("collapsed", info.token.properties == nil or info.token.properties.typeName ~= "monster")
			end,
			refreshToken = function(element, info)
...
function CharSheet.ChallengeRatingPanel()
	...
	resultPanel = gui.Panel{
		...
		refreshToken = function(element, info)
			element:SetClass("collapsed", info.token.properties.typeName ~= "monster")
		end,
		gui.Panel{
	...
	return gui.Panel{
		...
		gui.Panel{
			...
			gui.Button{
				...
				fontSize = 16,
				refreshToken = function(element, info)
					element:SetClass("collapsed", CharacterSheet.instance.data.info.token.properties.typeName ~= "monster")
				end,
				click = function(element)
					local newAbility = ActivatedAbility.Create{
		...
		CreatePanelFooter{
			...
			settings = function(element)
				...
				hitpointsTotalPanel = gui.Label{
					...
					textAlignment = "center",
					halign = "center",
					editable = creature.typeName == "monster",
					create = function(element)
