...
--The 5e panel set. Names must match DockablePanel registrations;
--unregistered names are skipped wherever the list is consumed, so it is
--safe for this list to be aspirational.
...
----------------------------------------------------------------------
--Codex panel renames do not apply to 5e; empty means the migration
--pass no-ops.
...
--The card face a character rail entry shows: portrait on top, hit
--points below, and (for characters) armor class. Shared by the docked
...
local function CreateCharacterCardVisual(charid)
    ...
    local UpdateCard = function()
        ...
        --5e: player characters show their armor class under the bar.
        local isCharacter = false
        pcall(function() isCharacter = c.typeName == "character" end)
        if heroRow ~= nil and heroRow.valid then
            heroRow:SetClass("collapsed", not isCharacter)
        end
        if isCharacter then
            local ac = 0
            pcall(function() ac = c:ArmorClass() or 0 end)
            if resourceLabel ~= nil and resourceLabel.valid then
                resourceLabel.text = tostring(ac)
            end
        end
    end
    ...
    resourceLabel = gui.Label{
        ...
        fontSize = 11,
        bold = true,
        --armor class; parchment to match the 5e look.
        color = "#d4d1ba",
        interactable = false,
        text = "",
    }
    heroRow = gui.Panel{
        ...
        gui.Panel{
            --shield glyph in front of the armor class number.
            bgimage = "phosphor/shield-fill.png",
            bgcolor = "#d4d1ba",
            width = 11,
            height = 11,
            valign = "center",
...
--Workspace Views (saved rail/dock arrangements) are not ported to 5e
--yet. All call sites are rawget-guarded, so their absence is safe.
