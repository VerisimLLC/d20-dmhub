...
function Hud.GetScreenHudArea(self)
	...
	local right = 0
	if gamehud ~= nil then
		if gamehud.leftDock ~= nil and (not gamehud.leftDock:HasClass("offscreen")) and #gamehud.leftDock.data.GetChildren() > 0 then
			left = (DockablePanel.DockWidth/1920) * (1920/1080) / (self.dialog.width/self.dialog.height)
		end
		if gamehud.rightDock ~= nil and (not gamehud.rightDock:HasClass("offscreen")) and #gamehud.rightDock.data.GetChildren() > 0 then
