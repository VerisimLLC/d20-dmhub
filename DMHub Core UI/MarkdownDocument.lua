...
--5e: the Draw Steel hardwired power-table presets (|easy / |medium / |hard)
--lived here; removed along with the power-roll parser.
...
BreakdownRichTags = function(content, result, options, extraOutput)
    ...
    for i, line in ipairs(lines) do
        ...
        --5e: the codex parses Draw Steel power-roll blocks here ("|Name: Attr"
        --plus tier lines). That mechanic doesn't exist in 5e, so the block is
        --removed; the display code downstream simply never sees a "power_roll"
        --token.
...
local function CreateMarkdownToolbar(opts)
    ...
    --5e: only tags with a registered widget here. The codex list also has
    --ability/encounter/party/follower (Draw Steel widgets, not ported) and a
    --"Draw Steel!" roll-link entry.
    local mediaTags  = { "image", "sound", "scene" }
