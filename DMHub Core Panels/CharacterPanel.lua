...
dmhub.GetSelectedMonster = function()
	...
	return gui.GetFocus().data.monsterid
end
...
local function CharacterDetailsPanel(token)
	...
	resultPanel = gui.Panel{
		...
		CharacterPanel.CreateCharacterDetailsPanel(token),
		...
		events = {
			...
			linger = function(element)
				if creature ~= nil then
					gui.Tooltip{ text = creature:ResistanceDescription(), textAlignment = 'center', valign = 'top' }(element)
				end
			end,
		},
					...
					gui.Panel{
							...
							events = {
								...
								linger = function(element)
									if currentToken ~= nil and currentToken.properties ~= nil then
										element.tooltip = gui.StatsHistoryTooltip{ description = "hitpoints", entries = currentToken.properties:GetStatHistory("hitpoints"):GetHistory()}
									end
								end,
						...
						gui.Label({
							...
							events = {
								...
								linger = function(element)
									if currentToken ~= nil and currentToken.properties ~= nil then
										element.tooltip = gui.StatsHistoryTooltip{ description = "maximum hitpoints", entries = currentToken.properties:GetStatHistory("max_hitpoints"):GetHistory()}
									end
								end,
							},
			...
			gui.Panel{
				...
				gui.Panel{
					...
					refreshCharacter = function(element, token)
						...
						if percent ~= element.data.targetPercent then
							element.data.targetPercent = percent

							if newToken then
								element:FireEvent("setwidth", percent)
					...
					animatefill = function(element)
						...
						if math.abs(delta) <= seekSpeed then
							element.data.animating = false
							element:FireEvent("setwidth", element.data.targetPercent)
							return
						end
					...
					setwidth = function(element, percent)
						...
						element:SetClass("bloodied", percent < 0.67)
						element:SetClass("critical", percent < 0.33)
					end,
				}
			},
...
local function CharacterDisplaySidePanel()
	local tokenPanels = {}
	local singleTokenDetailsPanel = nil
	local bestiaryPanel = nil
	if dmhub.isDM then
		bestiaryPanel = CreateBestiaryAndPartyPanel()
	...
	local resultPanel = gui.Panel{
		...
		refresh = function(element)
			...
			for _,token in ipairs(tokens) do
				if token.valid then
					local panel = tokenPanels[token.charid]
					if panel == nil or panel.data.token ~= token then
						panel = CharacterPanel.SingleCharacterDisplaySidePanel(token)
						createdNew = true
					end

					newTokenPanels[token.charid] = panel
			...
			if #tokens == 1 and #newChildren == 1 then
...
local CreateMonsterEntry = function(nodeid)
	...
	resultPanel = gui.Panel({
		...
		events = {
				...
				if panel ~= nil then
					element.tooltip = gui.TooltipFrame(
						panel,
						{
							...
							valign = "center",
                            vscroll = true,
                            maxHeight = dmhub.screenDimensions.y-20,
						}
					)
			...
			drag = mod.shared.CreateDragTargetFunction(node, function(nodeid) return assets:GetMonsterNode(nodeid) end, "Monsters"),
		...
		children = {
			...
			gui.Label({
				...
				refreshAssets = function(element)
					element.text = creature.GetTokenDescription(monster)
				end
			})
		}
	})
...
local CreateBestiaryFolder = function(nodeid)
	...
	folderPane = gui.Panel({
		...
		classes = {cond(isCollapsed, "collapsed-anim")},
					...
					menuItems[#menuItems+1] = {
						...
						click = function(element)
							...
							if numChildren == 0 then
								...
								local msg = string.format('Do you really want to delete %s and the %d monster entries within?', node.description, numChildren)
								if numChildren == 1 then
									msg = string.format('Do you really want to delete %s and the monster entry within?', node.description)
								end
...
CharacterPanel.CreateCharacterEntry = function(charid)
	...
	if creature == nil then
		print("CHARID::", charid, "has no properties.", traceback())
		return
	end
				...
				menuItems[#menuItems+1] = {
					...
					click = function(element)
						gamehud:ModalMessage{
							...
							message = "Are you sure you want to delete this character? They will be gone forever.",
							options = {
								{
									text = "Delete",
									execute = function()
	...
	local RefreshParty = function()
		...
		if partyid == nil then
			...
			for _,tok in ipairs(tokens) do
				partyMembers[#partyMembers+1] = tok.id
			end

			partyName = "Monsters/NPC's on Map"
		else
	...
	resultPanel = gui.Panel{
		...
		gui.Button{
			...
			press = function(element)
				...
				local addConditionButton = element
				local options = {}
				local ongoingEffectsTable = dmhub.GetTable("characterOngoingEffects") or {}
